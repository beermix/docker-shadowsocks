...
RUN apk upgrade --update \
  ...
  && ./autogen.sh \
  && ./configure CFLAGS="-march=native -O2 -pipe -fstack-protector-strong" CXXFLAGS="-march=native -O2 -pipe -fstack-protector-strong" CPPFLAGS="-D_FORTIFY_SOURCE=2" LDFLAGS="-Wl,-z,relro -Wl,-z,now" --prefix=/usr --disable-documentation --enable-shared --disable-static --enable-system-shared-lib --disable-ssp --disable-silent-rules \
  && make install \
  ...
  && ./autogen.sh \
  && ./configure CFLAGS="-march=native -O2 -pipe -fstack-protector-strong" CXXFLAGS="-march=native -O2 -pipe -fstack-protector-strong" CPPFLAGS="-D_FORTIFY_SOURCE=2" LDFLAGS="-Wl,-z,relro -Wl,-z,now" --disable-documentation --disable-ssp --disable-silent-rules \
  && make install \
  && cd /tmp \
