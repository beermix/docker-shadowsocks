...
RUN apk upgrade --update \
  ...
  libev-dev libsodium-dev mbedtls-static mbedtls-dev \
  ...
  && cmake -DCMAKE_BUILD_TYPE=Release -DWITH_DOC_HTML=0 -DWITH_DOC_MAN=0 -DWITH_EMBEDDED_SRC=0 -DWITH_SS_REDIR=0 -DWITH_STATIC=1 -DCMAKE_VERBOSE_MAKEFILE=1; make; mv bin/ss-server  /usr/bin/ \
  && cp ./bin/ss-*  /usr/bin/ss-server \
  && ls /usr/bin/ss-server | xargs -n1 setcap cap_net_bind_service+ep \
  && cd /tmp \
