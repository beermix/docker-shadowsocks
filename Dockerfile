...
RUN apk upgrade --update \
  ...
  && cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr -DUSE_SHARED_MBEDTLS_LIBRARY=0 -DUSE_STATIC_MBEDTLS_LIBRARY=1 -DENABLE_PROGRAMS=0 -DENABLE_TESTING=0 -DLINK_WITH_PTHREAD=1 -DCMAKE_C_FLAGS_RELEASE="-march=native -O3 -DNDEBUG -fPIC" -DCMAKE_VERBOSE_MAKEFILE=1 -Wno-dev \
  && make install \
  && cd /tmp \
